val dest_sorted = saClean.sortBy(_._5.toInt).map{case (arrDelay,deptDelay,dayOfWeek,dayofMonth,year,origin,dest,weatherDelay) => (dest,arrDelay,deptDelay)}
val delay_filtered = dest_sorted.filter(_._2 != "NA").filter(_._3 != "NA").map{case (a,b,c) => (a,(b.toInt,c.toInt,1))}
...
delay_avg.toDF("Destination","Avg_Arrival","Avg_Dept").show()
